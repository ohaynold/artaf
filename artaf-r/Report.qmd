...
something they foresee since there isn't enough certainty yet.
...
in their updates, with a variety of explanations of both phenomena.
...
forecasts themselves rather than in their updates. [@baars2005] However, 
one can easily 
...
that he has to include it. In that case, we'd simply see the change as an innovation
...
news and bad news. We may call a bias that overresponds to bad news
...
It should be noted that our question as posed does not directly answer anything about whether
the forecasts of the TAFs come true. That would require looking at actual weather
realizations, such as reported in METARs. We can thus not say how autocorrelative bias
...
for this being true). We merely can show whether knowledge of the history of the TAF predictions
...
(semi-, since they are reported as integers with rounding) continous variables 
...
Other variables may certainly be of importance, including binary ones like the forecast presence of 
thunderstorms or cumulonimbus clouds, but we hope that these continuous variables will gives us a good data set from which
...
The choice of these particular mappings is certainly somewhat arbitrary and much depends on kind of
...
the TAFs themselves also are not written toward one specific flight operation, but for general use for everyone
...
We can see that there clearly is a reversionary tendency in changes in TAFs, *i.e.*, if forecasters
...
(We intentionally do not show *p*-values as the rejection of a null hypothesis that there is not
a trace of autocorrelation in hundreds of millions of data points would be of very little interest
...
We can thus say that the changes in predictions exhibit a mixture of a pessimistic and an overreacting
bias. Forecasters overreact when they adjust forecasts, but while they overreact strongly for
worsening predictions, they overreact more mildly for improving predictions.
...
worsening predictions very likely to revert, whereas the equally treacherous skiing mountains around
...
at previously issued TAFs for the same time and place. If the prediction worsened, there is grounds for hope
that this worsening will be reversed in the final TAF before he goes flying. If the prediction improved,
there is grounds for hope that the improvement will mostly hold, though it is not very likely to be improved
further.
