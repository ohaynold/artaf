...
We break down the tendency of changes in predicted wind speed to revert by aerodrome. We see a cluster of a high reversionary tendency of increases in predicted wind speed to get reverted and a low tendency of decreases in predicted wind speed to get reverted in the Deep South. The opposite appears in the wind corridor from the Dakotas to Texas:
...
wind_speed_autocorr_aerodrome <- lines %>%
  ...
  mutate(change_increase=pmax(change_from_previous, 0),
         change_decrease=pmin(change_from_previous, 0)) %>%
  nest_by(aerodrome) %>%
  mutate(fitAerodrome = list(lm(change_to_final ~ change_increase + change_decrease + 0, data = data))) %>%
...
The geographical distribution of reversionary tendencies in the increases and decreases of predicted wind speeds suggests that these might be anticorrelated. This is indeed true:
...
Again, as with wind speed without gust, the Deep South stands out for its large tendency for increases in predicted wind speed with gust to revert and a low tendency for reversion of decreases in the predicted wind speed with gust:
