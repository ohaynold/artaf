...
wind_goodness<- function(wind_speed) { 
  (1 - 1 / (1 + exp(-0.14 * (wind_speed - 18)))) / (1 - 1 / (1 + exp(-0.14 * (0 - 18)))) 
  } 
...
```{r first-differences-goodness, warnings=FALSE}
changes_goodness <- lines_goodness %>%
  mutate(change_from_previous = current - previous,
         change_to_final = final - current) %>%
  group_by(aerodrome, year, variable, change_from_previous, change_to_final) %>%
...
print(
  ggplot(changes_goodness, aes(x=factor(round(change_from_previous*10,0)/10, levels=(-10:10)/10), y=change_to_final, weight=count)) +
...
correl_changes_goodness <- changes_goodness %>%
