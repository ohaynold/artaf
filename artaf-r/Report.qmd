...
in the sense of a Bayesian update. The direction of such biases found
appears inconsistent, however. The literature provides ample evidence
...
is a well-documented bias toward predicting rain. [@bickel2008] This
...
Studies about biases in weather forecasts have typically looked at bias
in the forecasts rather than bias in the updates to forecasts, but one
can easily hypothesize similar incentives for bias in updating
forecasts. For example, if consumers of a forecast are unhappy about the
...
: Goodness of `r sprintf("%.2f", wind_goodness(0))` for calm wind, `r sprintf("%.2f", wind_goodness(10))` for 10 knots, `r sprintf("%.2f", wind_goodness(20))` for 20 knots, `r sprintf("%.2f", wind_goodness(30))` for 30 knots, 0 for infinite wind.
...
observations of changes in TAFs for a given hour for each variable. Since most predictions span more than an hour, these observations are not independent (and, of course, even when the prediction changes between two
hours, the predictions for these hours come out of the same forecasting process). Over all variables,
`r format(sum(lines_goodness$count[lines_goodness$current!=lines_goodness$previous]), big.mark=",")` records
involve a change between the previous prediction and the current prediction.
...
#| fig-cap: Changes in predicted goodness of weather conditions
...
colnames(correl_changes_goodness_display) <- c("Variable", "Reversion of increases", "Reversion of decreases")
...
#| fig-cap: Reversion of Decreases in Goodness by Aerodrome
...
plot_usmap(fill="grey") +
  ...
  labs(title = "Reversion of Decreases in Predicted Goodness", colour="Reversion")
```
...
#| fig-cap: Reversion of Increases in Goodness by Aerodrome
plot_usmap(fill="grey") +
  geom_sf(data=usmap_transform(correl_changes_goodness_aerodrome %>% filter(term=="change_increase"), input_names = c("longitude", "latitude")),
             aes(colour=-estimate)) +
  scale_colour_gradientn(colours = heat.colors(10)) +
  labs(title = "Reversion of Increases in Predicted Goodness", colour="Reversion")
```
# Conclusion
...
of the forecasts, the relationship between that evolution and enventual weather realizations might be a fertile field for another study.
...
By station location, we see the well-known wind corridor from the
Dakotas to West Texas.
